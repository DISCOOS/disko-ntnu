...
public class JConsole extends JDialog {
	...
	private final String RX_EX = "RX: EXCEPTION: %1$s | %2$s";
	...
	private JPanel getContentPanel() {
		if (contentPanel == null) {
			...
			contentPanel.add(getOutputPanel(), BorderLayout.CENTER);
			contentPanel.add(getCommandPanel(), BorderLayout.SOUTH);			
		}
		return contentPanel;
	}
	...
	private IManagerListener listener = new IManagerListener() {
		...
		public void onBufferOverflow(ISession session, ProtocolEvent e) {
			publish("IOManager", String.format(RX_EX, 
					"Buffer overflow", session.getName()),"stream");
		}
