...
public abstract class AbstractMsoDs<M extends IMsoObjectIf, T extends IDsObjectIf> extends AbstractDiskoWork<Boolean>
	 ...
	 * be added.
	 */
	protected final ConcurrentLinkedQueue<MsoEvent.Update> m_queue =
		new ConcurrentLinkedQueue<MsoEvent.Update>();
	 ...
	 */
	protected final DataSourceImpl<DsEvent.Update> m_source;

	/**
	 * IDsUpdateListener repeater implementation
	 */
	protected final IDsUpdateListenerIf m_updateRepeater;
	...
	public AbstractMsoDs(Class<T> dataClass, String oprID, EnumSet<MsoClassCode> msoInterests, int dutyCycleTime, List<String> attributes) throws Exception {
		...
		m_source = new DataSourceImpl<DsEvent.Update>() {

			@SuppressWarnings("unchecked")
			public Collection<?> getItems(Class<?> c) {
				// only one type to return
				return AbstractMsoDs.this.getItems();
			}

		};

        // create source listener repeater
        m_updateRepeater = new IDsUpdateListenerIf() {

			@Override
			@SuppressWarnings("unchecked")
			public void handleDsUpdateEvent(DsEvent.Update e) {
				// forward
				m_source.fireSourceChanged(new SourceEvent<DsEvent.Update>(AbstractMsoDs.this,e));
			}

        };
        // connect repeater to client update events
        addUpdateListener(m_updateRepeater);
	...
	public void addSourceListener(ISourceListenerIf<DsEvent.Update> listener) {
		m_source.addSourceListener(listener);
	}

	public void removeSourceListener(ISourceListenerIf<DsEvent.Update> listener) {
	...
	protected void fireModified(T object) {
		fireUpdateEvent(new DsEvent.Update(this,DsEventType.MODIFIED_EVENT,0,new IDsObjectIf[]{object}));
	}
	...
	protected MsoEvent.Update getExisting(MsoEvent.Update e) {
		...
		for(MsoEvent.Update it : m_queue) {
	...
	private List<T> update(long tic) {
		...
		while(m_queue.peek()!=null) {
			...
			if(tic>0 && System.currentTimeMillis()-tic>maxTime)
				break;
