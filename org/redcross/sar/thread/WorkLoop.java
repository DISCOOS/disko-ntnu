...
public class WorkLoop extends AbstractWorkLoop {
	 ...
	 * Current work
	 */
	private IWork m_exec;

	/**
	 * Work queue
	 */
	private final ConcurrentLinkedQueue<IWork> m_queue = new ConcurrentLinkedQueue<IWork>();
	...
	public synchronized int doWork() {
		...
		while(duration < m_timeOut && m_queue.peek()!=null) {
			...
			try {
				 ...
				 *
				 * ============================================= */
				if(getThreadType().equals(ThreadType.WORK_ON_SAFE) || m_exec.isSafe()) {
					// do the work
					m_exec.run();
				}
			...
			if(m_exec.isLoop()) loops.add(m_exec);

			// get next in queue?
			if(duration < m_timeOut) m_exec = m_queue.poll();
