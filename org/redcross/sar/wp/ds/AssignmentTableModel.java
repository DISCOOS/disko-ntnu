...
public class AssignmentTableModel extends DsObjectTableModel<RouteCost> {
	...
	private static final String[] CAPTIONS = 
		...
		new String[]{	"Oppdrag","Status","Enhet",
	...
	public AssignmentTableModel(IDsIf<RouteCost> ds,boolean archived) {

		// forward
		super(ds, ATTRIBUTES, CAPTIONS, TOOLTIPS);
	...
	protected void setDefaults() {
		if(m_archived) 
			setHeaderEditable(ARCHIVED_COLUMNS[ARCHIVED_COLUMNS.length-1], true);
		else
			setHeaderEditable(ACTIVE_COLUMNS[ACTIVE_COLUMNS.length-1], true);
	}
	...
	protected Object getDsAttrValue(IDsObjectIf dsObj, String name) {
		if(NAME.equals(name)) {
			...
			return ((IAssignmentIf)dsObj.getId()).getOwningUnit();
		}
		// failed
		return null;
	}
	...
	public boolean select(RouteCost dsObj) {
		if(dsObj!=null ) {
			// get history flag
			boolean isHistory = dsObj.isArchived();
			// finished
	...
	public boolean isActive(int row) {
		IAssignmentIf assignment = (IAssignmentIf)getValueAt(row, NAME_INDEX);
