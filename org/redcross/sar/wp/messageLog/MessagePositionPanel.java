...
public class MessagePositionPanel extends BasePanel implements IEditorIf
{
	...
	public MessagePositionPanel(IDiskoWpMessageLog wp)
	{
		...
		// prepare
		m_wp = wp;
		m_tool = wp.getApplication().getNavMenu().getPositionTool();
	...
	private PositionPanel getPositionPanel() {
		if(m_positionPanel==null) {
			...
			m_positionPanel.addWorkListener(this);
	...
	private DTGField getDTGAttr() {
		if (m_dtgAttr == null) {
			// get from position panel
			m_dtgAttr = (DTGField)getPositionPanel().getOptionsPanel().getField("DTG");
	...
	private boolean apply()
	{
		...
		// update position
		line.setLineTime((Calendar)getDTGAttr().getValue());
		line.setLinePosition(panel.getPosition());

		// update log entry
		panel.setLogEntry(new TimePos( line.getLinePosition(), line.getLineTime()));
	...
	private void update(IMessageIf message)
	{
		...
		try
		{
			...
			if(line==null) {
				...
				// get log time stamp
				Calendar time = line.getLineTime();
				// set log time stamp
		...
		if(message != null)
		{
			...
			if(unit != null)
			{
				try
				{
					...
					if(isSelected) map.centerAtMsoObject(unit);
